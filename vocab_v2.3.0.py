...
class MainWindow(QtWidgets.QMainWindow):
    def __init__(self):
        ...
        btn_row.addWidget(self.btn_go_study, 1)

        self.btn_review_selected = QtWidgets.QPushButton("合并复习(选中)")
        self.btn_review_selected.setObjectName("accent")
        self.btn_review_selected.setFixedHeight(44)
        self.btn_review_selected.clicked.connect(self.start_review_selected_units)
        btn_row.addWidget(self.btn_review_selected, 1)
    ...
    # 3) MainWindow：实现多选读取 + 合并复习入口 + 传递“不过滤到期”开关
    ...
    def start_review_selected_units(self):
        ...
        if not units:
            QtWidgets.QMessageBox.information(self, "提示", "请先在左侧选择一个或多个单元。")
            return
        # 默认“不过滤到期”，保证点开就学
        self.open_study_window(unit_filter=units, include_all=True)

    def open_study_window(self, unit_filter=None, include_all=True):
        # 默认为右侧下拉框选择；include_all 固定为 True（已删除复选框）
        if unit_filter is None:
            sel = (self.study_unit_select.currentText() or "").strip()
            unit_filter = None if sel in ("所有单元", "") else sel
        self.study_win = StudyWindow(self.db, unit_filter, include_all=include_all)
        self.study_win.show()
    # ---------- 单元管理 ----------
    ...
    # 4) 替换 MainWindow._impl_auto_fill_kana_from_term，放宽自动覆盖条件并维护 _last_auto_romaji
