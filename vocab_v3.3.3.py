...
from kana_quiz import KanaQuiz
...
def romaji_to_kana(s: str) -> tuple[str, str]:
    """
    连续罗马音 → (平假名, 片假名)
    规则满足你的要求：
    - ん 直接输入 'n' 即可（不需要 'nn'）
    - つ 输入 'tsu'；促音输入 'xtsu'（得到 'っ'）
    - 小写拗音/元音用前缀 'x'：'xya' 'xyu' 'xyo'，以及 'xa' 'xi' 'xu' 'xe' 'xo'
    - 对于 'きょ'，既可 'kyo'，也可 'kixyo'（'ki' + 'xyo'）
    """
    s = (s or "").strip().lower()
    i, n = 0, len(s)
    out: list[str] = []
    ...
    vow = {"a": "あ", "i": "い", "u": "う", "e": "え", "o": "お"}

    while i < n:
        # 1) x-前缀的小写
        if s[i] == "x":
            # 先试四/三字符（xtsu / xya/xyu/xyo / xa…）
            if i + 4 <= n and s[i:i+4] in small:
                out.append(small[s[i:i+4]]); i += 4; continue
            if i + 3 <= n and s[i:i+3] in small:
                out.append(small[s[i:i+3]]); i += 3; continue
            # 单个 'x' 无法识别时，原样跳过
            i += 1
            continue

        # 2) 拗音三拼
        if i + 3 <= n and s[i:i+3] in yoon:
            out.append(yoon[s[i:i+3]]); i += 3; continue

        # 3) 特例三拼
        if i + 3 <= n and s[i:i+3] in special3:
            out.append(special3[s[i:i+3]]); i += 3; continue

        # 4) 单元音
        if s[i] in vowels:
            out.append(vow[s[i]]); i += 1; continue

        # 5) 基本 CV（含 si/ti/tu 等按表走）
        if i + 2 <= n:
            c, v = s[i], s[i+1]
            if c in row and v in vowels:
                kana = row[c][vowels[v]]
                if kana:
                    out.append(kana); i += 2; continue

        # 6) 单独的 'n' → ん（放到最后，保证 'na/ni/nya' 已被上面的规则消费）
        if s[i] == "n":
...
def _hira_to_kata(s: str) -> str:
    ...
    for ch in s or "":
        ...
        if 0x3041 <= code <= 0x3096:
            out.append(chr(code + 0x60))  # ひらがな→カタカナ
        else:
            out.append(ch)
    return "".join(out)
...
QWidget, QMenuBar, QMenu, QToolTip {
    /* 先用 Hanalei；缺字时回退到 Zen Maru（假名/日文），
       再回退到通用 CJK，避免同一句出现“半截换字体”的现象 */
    font-family: "Hanalei", "Zen Maru Gothic",
...
QLabel#sectionTitle {
    ...
    font-family: "Zen Maru Gothic","Microsoft YaHei","PingFang SC","Noto Sans CJK SC","SimHei", Arial;
}
...
/* 收窄右侧下拉按钮区域，进一步压低视觉高度 */
QComboBox#sortCombo::drop-down {
    width: 22px;
}

/* 下拉列表项也做小一点，保持整体协调 */
QComboBox#sortCombo QAbstractItemView::item {
    padding: 4px 8px;
    min-height: 20px;
}
"""

# 1) 在现有 APP_STYLE 末尾继续追加（覆盖之前的按钮字重）
APP_STYLE += """
/* —— 字体发糊修正（Windows 合成粗体） —— */
/* 全局：移除 Hanalei 作为通用字体，统一用 CJK 家族 */
QWidget, QMenuBar, QMenu, QToolTip {
    font-family: "Zen Maru Gothic","Microsoft YaHei UI","PingFang SC","Noto Sans CJK SC","SimHei", Arial;
}

/* 仅标题继续允许 Hanalei（英文字形），不影响中文 */
QLabel#appTitle {
    font-family: "Hanalei","Zen Maru Gothic","Microsoft YaHei UI","PingFang SC","Noto Sans CJK SC","SimHei", Arial;
}

/* 按钮：明确使用 CJK 字体并取消粗体，避免合成粗体发糊 */
QPushButton#primary,
QPushButton#accent,
QPushButton#secondary,
QPushButton#mini,
QPushButton#miniDanger {
    font-family: "Microsoft YaHei UI","Zen Maru Gothic","PingFang SC","Noto Sans CJK SC","SimHei", Arial;
    font-weight: normal;  /* 关键：去掉 600/700，防止回退字体被合成粗体 */
}
...
# 1) 在 APP_STYLE 定义的末尾追加（放在所有 APP_STYLE += ... 之后）
APP_STYLE += """
/* —— 全局取消粗体 —— */
QWidget, QMenuBar, QMenu, QToolTip, QLabel, QPushButton, QCheckBox, QRadioButton,
QGroupBox, QLineEdit, QComboBox, QTextEdit, QListWidget, QTableWidget, QHeaderView::section {
    font-weight: normal;
}

/* 覆盖之前设为粗体的控件与 ID（使用相同选择器确保覆盖生效） */
QPushButton#primary,
QPushButton#accent,
QPushButton#secondary,
QPushButton#mini,
QPushButton#miniDanger { font-weight: normal; }

QLabel#appTitle,
QLabel#sectionTitle,
QLabel#bigterm { font-weight: normal; }
"""
...
class MainWindow(QtWidgets.QMainWindow):
    def __init__(self):
        ...
        # —— 新增：罗马音输入法（不入库） + 两个候选按钮 ——
        self.roma_ime = QtWidgets.QLineEdit()
        self.roma_ime.setPlaceholderText("在此连续输入罗马音，例如: shixyuxtsushin（不保存到词条）")
        form_col.addRow("罗马音(输入法，不保存)", self.roma_ime)

        # 两个候选按钮（平/片），点击将候选一次性写回“假名”
        ...
        _btnrow_h.setContentsMargins(0, 0, 0, 0)
        _btnrow_h.setSpacing(6)
        _btnrow_h.addWidget(self._btn_kana_hira, 0)
        _btnrow_h.addWidget(self._btn_kana_kata, 0)
        form_col.addRow("候选", _btnrow)

        # 绑定：输入法文本变化 → 刷新候选；回车默认取“平假名”候选
        self.roma_ime.textEdited.connect(self._on_roma_ime_edited)
        self.roma_ime.returnPressed.connect(self._commit_roma_enter)

        # 绑定：点击两个候选按钮 → 把对应假名写回 add_term
        ...
        # —— 中文释义 → 自动补全（去抖 300ms）——
        self._mean2jp_timer = QtCore.QTimer(self)
        self._mean2jp_timer.setSingleShot(True)
        ...
        # —— 假名/汉字 的轻量补全（可选）——
        try:
            # 从词典构造候选字符串（各取若干，避免太大）
            kana_list = []
            kanji_list = []
            for (_k, (term, kana, mean)) in list(_LOCAL_DICT._entry.items())[:20000]:
                if kana and kana not in kana_list: kana_list.append(kana)
                if term and any(0x4E00 <= ord(c) <= 0x9FFF for c in term) and term not in kanji_list:
                    kanji_list.append(term)

            # 保存全集（用于重排）
            self._kana_all = kana_list[:]
            self._kanji_all = kanji_list[:]

            # KANA
            self._kana_model = QtCore.QStringListModel(self._kana_all, self)
            comp_kana = QtWidgets.QCompleter(self._kana_model, self.add_term)
            comp_kana.setCaseSensitivity(QtCore.Qt.CaseInsensitive)
            comp_kana.setFilterMode(QtCore.Qt.MatchContains)
            comp_kana.setCompletionMode(QtWidgets.QCompleter.PopupCompletion)
            self.add_term.setCompleter(comp_kana)
            self.add_term.textEdited.connect(self._reorder_kana_model)

            # KANJI
            self._kanji_model = QtCore.QStringListModel(self._kanji_all, self)
            comp_kanji = QtWidgets.QCompleter(self._kanji_model, self.add_kanji)
            comp_kanji.setCaseSensitivity(QtCore.Qt.CaseInsensitive)
            comp_kanji.setFilterMode(QtCore.Qt.MatchContains)
            comp_kanji.setCompletionMode(QtWidgets.QCompleter.PopupCompletion)
            self.add_kanji.setCompleter(comp_kanji)
            self.add_kanji.textEdited.connect(self._reorder_kanji_model)
        ...
        tools_bar.setContentsMargins(0, 0, 0, 4)  # 下边缘收紧一点，整体更贴合
        tools_bar.setSpacing(6)  # 8 → 6，更紧凑
        ...
        self.sort_combo.setObjectName("sortCombo")  # 指定唯一 ID，便于样式精确覆盖
        self.sort_combo.setFixedHeight(26)  # 保险起见，先收一收高度
        ...
        self.sort_combo.setSizePolicy(QtWidgets.QSizePolicy.Fixed, QtWidgets.QSizePolicy.Fixed)

        for b in (self.btn_shuffle, self.btn_overview):
            b.setFixedHeight(28)
        ...
        self.search_box.setSizePolicy(QtWidgets.QSizePolicy.Fixed, QtWidgets.QSizePolicy.Fixed)
        tools_bar.addWidget(self.search_box)

        # 保留对象以兼容旧代码，但不再占空间
        center_layout.addWidget(self.overview)
        self.overview.setMaximumHeight(0)
        self.overview.setVisible(False)

        center_layout.addLayout(tools_bar)

        # 表格（QTableView + Model + Delegate）
        self.unit_table = QtWidgets.QTableView()

        # 模型 & 代理（可排序）
        self._card_model = CardTableModel(self)
        self._proxy = CardSortProxy(self)  # 使用自定义代理
        self._proxy.setSourceModel(self._card_model)
        self._proxy.setSortRole(QtCore.Qt.DisplayRole)
        self.unit_table.setModel(self._proxy)
        self.unit_table.setSortingEnabled(False)

        # 【稳定方案】操作列改用 setIndexWidget 放两个真实按钮（见 _rebuild_op_column）
        self._op_delegate = None
        ...
        self.unit_table.setColumnWidth(5, 84)  # 4+28+6+28+4=70，留余量到 84，不会遮
        ...
        # 应用持久化尺寸
        self.unit_table.verticalHeader().setDefaultSectionSize(int(self.settings.get("table_row_height", 28)))
        self.unit_table.setColumnWidth(5, int(self.settings.get("op_col_width", 84)))

        # 可选：全局字号缩放（温和处理）
        fs = float(self.settings.get("font_scale", 1.0) or 1.0)
        try:
            base_px = 12
            self.setStyleSheet(f"* {{ font-size: {int(base_px * fs)}px; }}")
        except Exception:
            pass
    ...
    def _rebuild_op_column(self):
        """
        为“操作”列（第 5 列，0 基）放置两个图标按钮：编辑 / 删除。
        ...
        for r in range(rows):
            ...
            btn_del.clicked.connect(lambda _=False, x=cid: self._delete_card_by_id(x))
    ...
    def open_kana_quiz(self, _checked=False):
        ...
        if not hasattr(self, "_kana_quiz_window") or self._kana_quiz_window is None:
            from kana_quiz import KanaQuiz
            self._kana_quiz_window = KanaQuiz(None)  # 顶级窗口，避免继承 MainWindow 的样式
    ...
    def open_kana_quiz(self):
        try:
            from kana_quiz import KanaQuiz
            win = KanaQuiz(None)  # 改为顶级窗口，避免继承 MainWindow 的样式

            # 把数据库句柄传过去，便于题库取数（如果你的 kana_quiz 里需要）
            try:
                win.db = self.db
            except Exception:
                pass
            # 防止被 Python 垃圾回收：挂到 self 上
            if not hasattr(self, "_subwins"): self._subwins = []
            self._subwins.append(win)
            win.show()
        except Exception as e:
            QtWidgets.QMessageBox.critical(self, "错误", f"无法打开测验窗口：{e}")
...
class StudyWindow(QtWidgets.QWidget):
    def __init__(self, db, unit_filter=None, include_all=False):
        ...
        self._rated = False  # 是否已评分/已判定（防止重复计数）
        self._ts_start = 0.0  # 本题计时起点（monotonic）
        self._rated = False
    ...
    def show_card(self, card):
        ...
        # 控制评分按钮可见性：日→中可见，但初始禁用，待展开释义后启用
        for b in (self.btn_again, self.btn_hard, self.btn_good, self.btn_easy):
            b.setVisible(mode == 0)
            b.setEnabled(False)
    ...
    def check_answer(self):
        ...
        # 记录 reviews：正确=4（good），错误=1（again）。你也可以按需设 3/5。
        quality = 4 if correct else 1
        elapsed = int((time.monotonic() - self._ts_start) * 1000) if self._ts_start else None
        self._record_review(quality, elapsed_ms=elapsed)

        # —— 新增：若未匹配且用户输入是“汉字”，用本地词典反查假名来兜底判定 ——
        if (not correct) and _has_kanji(user_input):
            # 题面里能取到的“标准假名”（优先专用列，没有就回退 term）
            ...
            if std_kana:
                rec = None
                try:
                    # 你工程里已集成的本地词典类 LocalJaZhDict
                    rec = _LOCAL_DICT.get_full(user_input)
                except Exception:
                    rec = None

                # 词典命中后拿到“该汉字写法”的假名，与标准假名比对
                if rec:
                    # rec 结构：(term, kana, meaning)
                    _term2, kana2, _mean2 = rec
                    if norm(kana2) == std_kana:
                        correct = True
...
# —— 放在 def main() 之前，靠近其它工具函数处 ——

# 2) 调整默认应用字体：不要用 Hanalei 做默认，按平台选更合适的 CJK 字体
def _install_app_fonts(app: QtWidgets.QApplication):
    base_dir = os.path.dirname(os.path.abspath(__file__))
    path_zen = os.path.join(base_dir, "ZenMaruGothic-Medium.ttf")
    path_han = os.path.join(base_dir, "Hanalei-Regular.ttf")

    fdb = QtGui.QFontDatabase()

    fam_zen = "Zen Maru Gothic"
    try:
        if os.path.exists(path_zen):
            fdb.addApplicationFont(path_zen)
    except Exception:
        pass

    fam_han = "Hanalei"
    try:
        if os.path.exists(path_han):
            fdb.addApplicationFont(path_han)
    except Exception:
        pass

    # 回退链：让 Hanalei 回退到 CJK，不作为默认
    QtGui.QFont.insertSubstitution(fam_han, fam_zen)
    for fb in ("Microsoft YaHei UI", "PingFang SC", "Noto Sans CJK SC", "SimHei", "Arial"):
        QtGui.QFont.insertSubstitution(fam_han, fb)

    # 关键：平台默认字体
    fam_base = "Microsoft YaHei UI" if sys.platform.startswith("win") else fam_zen
    app.setFont(QtGui.QFont(fam_base, 10))

    # 让 matplotlib 也使用 CJK 字体
    try:
        import matplotlib
        matplotlib.rcParams["font.sans-serif"] = [
            fam_zen, "Microsoft YaHei UI", "Noto Sans CJK JP", "Noto Sans CJK SC", "SimHei", "Arial"
        ]
        matplotlib.rcParams["axes.unicode_minus"] = False
    except Exception:
        pass
...
def main():
    ...
    _install_app_fonts(app)            # ← 新增：安装并启用字体与回退链
    app.setStyleSheet(APP_STYLE)       # 样式表里也已把字体/字号改好
    w = MainWindow()
    w.show()
    sys.exit(app.exec_())
