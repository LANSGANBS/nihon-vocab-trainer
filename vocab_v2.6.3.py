...
class LocalJaZhDict:
    ...
    def search_by_meaning(self, zh: str, limit: int = 5) -> list[tuple[str, str, str]]:
        """
        通过中文释义反查条目，返回若干 (term, kana, meaning) 候选，按“优先级”排序：
        1) 完全等于 > 前缀匹配 > 子串匹配
        2) 有 kana 的优先
        3) term 含汉字的优先（便于同时补“汉字写法”）
        """
        if not self._loaded:
            self.load()
        q = (zh or "").strip()
        ...
        for _k, tup in self._entry.items():
            ...
            if kana: score += 3
            if term and any(0x4E00 <= ord(c) <= 0x9FFF for c in term):  # 是否含汉字
                score += 2
...
class MainWindow(QtWidgets.QMainWindow):
    def __init__(self):
        ...
        self._mean2jp_timer.timeout.connect(self._auto_fill_from_zh)
        ...
        self.unit_table.setColumnWidth(5, 130)  # 操作列固定足够宽度
    ...
    def populate_unit_table(self, rows):
        ...
        for row_idx, r in enumerate(rows):
            ...
            card_id = int(r[0]) if (r and r[0] is not None) else -1
            btn_edit.setProperty("card_id", card_id)
            btn_del.setProperty("card_id", card_id)

            btn_edit.clicked.connect(self.edit_card_dialog)
            btn_del.clicked.connect(self.delete_card_dialog)

            ops_widget.setMinimumWidth(120)
            ...
            self.unit_table.setColumnWidth(5, 130)
...
class StudyWindow(QtWidgets.QWidget):
    ...
    def keyPressEvent(self, e: QtGui.QKeyEvent):
        ...
        if key in (QtCore.Qt.Key_1, QtCore.Qt.Key_2, QtCore.Qt.Key_3, QtCore.Qt.Key_4):
            # 已评分则忽略，防止重复计数
            if getattr(self, "_rated", False):
                return
            # 未展开先展开，不直接给分
            if not getattr(self, "showing_mean", False):
                self.toggle_meaning()
                return
            if self.current and self.current[1] == 0:  # 仅日→中评分
                mp = {
                    QtCore.Qt.Key_1: 1,
                    QtCore.Qt.Key_2: 3,
                    QtCore.Qt.Key_3: 4,
    ...
    def toggle_meaning(self):
        ...
        if self.showing_mean:
            ...
            if mode == 0:
                # 仅在“尚未评分”时允许继续评分
                for b in (self.btn_again, self.btn_hard, self.btn_good, self.btn_easy):
                    b.setEnabled(not getattr(self, "_rated", False))
                # 评分前不允许继续
                self.continue_btn.setEnabled(False)
                self.continue_btn.setDefault(True)
                self.continue_btn.setFocus()
        ...
        # 隐藏 -> 展开
        if mode == 0:
            # 日→中：展开仅显示中文释义；启用评分；继续按钮保持禁用（待评分）
            zh = (row[4] or "").strip()
            self.mean_label.setText(zh)
            for b in (self.btn_again, self.btn_hard, self.btn_good, self.btn_easy):
                b.setEnabled(not getattr(self, "_rated", False))
