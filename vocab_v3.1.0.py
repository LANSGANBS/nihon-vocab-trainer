...
def _init_mpl_cjk_fonts(extra_font_paths=None):
    ...
    try:
        ...
        if os.path.isdir(local_fonts_dir):
            for fp in glob.glob(os.path.join(local_fonts_dir, "*.ttf")) + glob.glob(os.path.join(local_fonts_dir, "*.otf")):
                try:
                    fm.fontManager.addfont(fp)
                except Exception:
                    pass
...
class CardTableModel(QtCore.QAbstractTableModel):
    ...
    def data(self, index, role):
        ...
        # 供 sort/filter 使用的“真实值”
        if role == self._sort_role:
            if col == 0:  # 序号：按源模型行号排序键，而非 DB id
                return index.row()
            elif col == 1:   # 假名
                return (r[3] or "").strip()
            elif col == 2:   # 汉字
                return (r[11] or "").strip()
            elif col == 3:   # 罗马音
                return (r[12] or "").strip()
            elif col == 4:   # 释义
                return (r[4] or "").strip()
            else:            # 操作
                return ""
...
class MainWindow(QtWidgets.QMainWindow):
    def __init__(self):
        ...
        self.add_mean.textChanged.connect(lambda _=None: self._mean2jp_timer.start(300))
    ...
    def _on_unit_list_menu(self, pos: QtCore.QPoint):
        ...
        if chosen == act_rename and item:
            old = item.text().strip()
            if old == "所有单元":
                try:
                    self._msg_warn("无法重命名", "请先选中一个具体单元。")
                except Exception:
    ...
    def _apply_candidate_tuple(self, cand: tuple[str, str, str], force: bool = False):
        ...
        # 3) 假名：优先用 kana；若缺 kana 且 term 是纯假名，则用 term
        kana_to_use = kana if kana else (term if self._is_kana_only(term or "") else "")
        if kana_to_use:
    ...
    def _apply_candidate_from_popup(self, cand: tuple[str, str, str], remember: bool):
        ...
        self._apply_candidate_tuple(cand, force=False)
        if remember and zh:
    ...
    def refresh_units(self):
        ...
        # 并入会话级临时单元
        if hasattr(self, "_adhoc_units"):
            for u in sorted(self._adhoc_units):
                if u not in got:
                    self.unit_list.addItem(u)
    ...
    def create_unit_dialog(self):
        ...
        if ok and text.strip():
            name = text.strip()
            self.add_unit_combo.setEditText(name)
    ...
    def add_card_from_form(self):
        ...
        if not unit:
            QtWidgets.QMessageBox.warning(
                self, "未选择单元",
                "请先在左侧选择一个单元，或在下方“单元”输入框中输入新单元名。"
            )
            self.add_unit_combo.setFocus()
            return
...
class SettingsDialog(QtWidgets.QDialog):
    def __init__(self, parent, settings):
        ...
        # 视图/UI
        self.spin_row_h = QtWidgets.QSpinBox(); self.spin_row_h.setRange(20, 60)
        self.spin_row_h.setValue(int(settings.get("table_row_height", 28)))
        form.addRow("表格行高", self.spin_row_h)

        self.spin_op_w = QtWidgets.QSpinBox(); self.spin_op_w.setRange(60, 200)
        ...
        self.dspin_font.setDecimals(1); self.dspin_font.setRange(0.8, 1.6); self.dspin_font.setSingleStep(0.1)
        ...
        row.addWidget(self.ed_dict); row.addWidget(btn)
...
class StatsDialog(QtWidgets.QDialog):
    ...
    def __init__(self, parent, db, use_mpl=True):
        ...
        # 总览 tab
        self.tab_overview = QtWidgets.QWidget(); self.tabs.addTab(self.tab_overview, "总览")
        self._build_overview_tab()

        # 活动 tab
        self.tab_activity = QtWidgets.QWidget(); self.tabs.addTab(self.tab_activity, "复习活动")
        self._build_activity_tab()

        # 质量 tab
        self.tab_quality = QtWidgets.QWidget(); self.tabs.addTab(self.tab_quality, "记忆质量")
    ...
    def _build_activity_tab(self):
        ...
        if self.use_mpl:
            ...
            for c in (1,2): self.tbl_act.horizontalHeader().setSectionResizeMode(c, QtWidgets.QHeaderView.ResizeToContents)
    ...
    def _make_canvas(self, parent_layout, title: str):
        w = QtWidgets.QWidget()
        v = QtWidgets.QVBoxLayout(w); v.setContentsMargins(0,0,0,0)
        lb = QtWidgets.QLabel(title); v.addWidget(lb)
        fig = plt.figure()
    ...
    def _load_data(self):
        ...
        self.reviews_per_day = cur.execute(
            ...
            "FROM reviews GROUP BY d ORDER BY d DESC LIMIT 90"
        ).fetchall()
