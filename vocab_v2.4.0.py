...
class MainWindow(QtWidgets.QMainWindow):
    ...
    def _auto_fill_meaning_from_term(self, force: bool = False):
        ...
        def _is_japanese_like(s: str) -> bool:
            if not s:
                return False
            for ch in s:
                code = ord(ch)
        ...
        keys = [k for k in raw_keys if _is_japanese_like(k)]
        if not keys:
            return
    ...
    def populate_unit_table(self, rows):
        ...
        for row_idx, r in enumerate(rows):
            ...
            # 2) 日语词条（合并显示：term \| jp_kanji）
            term_item = QtWidgets.QTableWidgetItem(format_jp_term_for_table(r))
            self.unit_table.setItem(row_idx, 1, term_item)

            # 3) 罗马音（尽量从 jp_kana 转；若无则留空）
            ...
            self.unit_table.setItem(row_idx, 2, romaji_item)
            ...
            h.addWidget(btn_del)
            h.addStretch()
            self.unit_table.setCellWidget(row_idx, 4, ops_widget)
...
class EditDialog(QtWidgets.QDialog):
    def __init__(self, parent, conn, row):
        ...
        for cb in (getattr(self, "cb_lang", None), getattr(self, "cb_unit", None)):
