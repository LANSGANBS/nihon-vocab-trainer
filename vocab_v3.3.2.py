...
def _install_app_fonts(app: QtWidgets.QApplication):
    """安装 Hanalei / ZenMaruGothic，并设置全局默认字体与回退链。"""
    base_dir = os.path.dirname(os.path.abspath(__file__))
    path_zen = os.path.join(base_dir, "ZenMaruGothic-Medium.ttf")
    path_han = os.path.join(base_dir, "Hanalei-Regular.ttf")  # 需要 TTF/OTF
    ...
    try:
        if os.path.exists(path_zen):
            fid = fdb.addApplicationFont(path_zen)
            fams = fdb.applicationFontFamilies(fid)
            if fams:
                fam_zen = fams[0]
    except Exception:
        pass
    ...
    try:
        if os.path.exists(path_han):
            fid2 = fdb.addApplicationFont(path_han)
            fams2 = fdb.applicationFontFamilies(fid2)
            if fams2:
                fam_han = fams2[0]
    except Exception:
        pass

    # 3) 建立“回退链”：以 Hanalei 为主；无字形时优先回退到 ZenMaru（假名/日文），
    #    再回退到系统 CJK，保证中文等也不乱码、且整句观感一致
    QtGui.QFont.insertSubstitution(fam_han, fam_zen)
    for fb in ("Microsoft YaHei", "PingFang SC", "Hiragino Sans GB",
               "Noto Sans CJK SC", "SimHei", "Arial"):
        QtGui.QFont.insertSubstitution(fam_han, fb)

    # 4) 设置“应用默认字体”为 Hanalei（字号配合样式表 16px）
    app.setFont(QtGui.QFont(fam_han, 10))

    # 5) 让 matplotlib 也用到 CJK 字体（你的统计图会读这个）
    try:
        import matplotlib
        matplotlib.rcParams["font.sans-serif"] = [
            fam_zen, "Microsoft YaHei", "PingFang SC", "Hiragino Sans GB",
            "Noto Sans CJK SC", "SimHei", "Arial"
        ]
        matplotlib.rcParams["axes.unicode_minus"] = False
    except Exception:
